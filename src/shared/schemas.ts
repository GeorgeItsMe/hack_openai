...
export const requestSchema = z.object({ kind: z.enum(['classify', 'task', 'groups', 'next', 'summary', 'chat']), context: contextSchema }).strict().refine(v => v.kind !== 'chat' || !!v.context.chat, { message: 'Chat context is required' });
export type AIRequest = z.infer<typeof requestSchema>;
export const outputSchemas = { classify: assessmentSchema, task: taskSchema, groups: groupSchema, next: nextSchema, summary: summarySchema, chat: chatSchema };
