...
export const contextSchema = z.object({
  language: z.enum(['ru', 'en']), goal: z.string().max(1000).default(''), task: z.string().max(2000).default(''),
...
export const requestSchema = z.object({ kind: z.enum(['classify', 'task', 'groups', 'next', 'summary']), context: contextSchema }).strict();
export type AIRequest = z.infer<typeof requestSchema>;
export const outputSchemas = { classify: assessmentSchema, task: taskSchema, groups: groupSchema, next: nextSchema, summary: summarySchema };
