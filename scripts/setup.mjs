...
if (!/^FOCUSTAB_PAIR_TOKEN=.{32,}$/m.test(env)) {
  const line = `FOCUSTAB_PAIR_TOKEN=${randomBytes(32).toString('hex')}`;
  env = /^FOCUSTAB_PAIR_TOKEN=.*$/m.test(env) ? env.replace(/^FOCUSTAB_PAIR_TOKEN=.*$/m, line) : env + '\n' + line + '\n';
}
await writeFile('.env', env, { mode: 0o600 }); await chmod('.env', 0o600);
await mkdir('.local', { recursive: true });
const token = env.match(/^FOCUSTAB_PAIR_TOKEN=(.*)$/m)[1].trim();
